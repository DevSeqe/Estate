...
abstract class elFinderVolumeDriver {
    ...
    public function duplicate($hash, $suffix = 'copy') {
        ...
        return ($path = $this->compress($file)) == false ? false : $this->stat($path);
    }
    ...
    public function search($q, $mimes) {
        return $this->commandDisabled('search') ? array() : $this->doSearch($this->root, $q, $mimes);
    }
    ...
    protected function updateCache($path, $stat) {
        ...
        if ($root) {
            unset($stat['hidden']);
        } elseif ($this->attr($path, 'hidden', isset($stat['hidden']) ? !!$stat['hidden'] : null) || !$this->mimeAccepted($stat['mime'])) {
            $stat['hidden'] = 1;
        } else {
            unset($stat['hidden']);
        }
